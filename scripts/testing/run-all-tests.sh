...
echo -e "${CYAN}🧪 Neo Service Layer - Comprehensive Test Suite${NC}"
echo -e "${CYAN}=================================================${NC}"
echo ""
...
echo -e "${YELLOW}🔍 Discovering test projects...${NC}"
TEST_PROJECTS=$(find tests -name "*.csproj" -type f | sort)
TEST_COUNT=$(echo "$TEST_PROJECTS" | wc -l)

echo -e "${BLUE}Found ${TEST_COUNT} test projects:${NC}"
...
if [ $FAILED_PROJECTS -eq 0 ]; then
    echo -e "${GREEN}🎉 All test projects passed!${NC}"
else
