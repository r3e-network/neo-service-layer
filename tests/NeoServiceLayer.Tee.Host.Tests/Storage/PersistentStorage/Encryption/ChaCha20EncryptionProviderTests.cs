...
{
    public class ChaCha20EncryptionProviderTests : IDisposable
    {
        private readonly Mock<ILogger<ChaCha20EncryptionProvider>> _loggerMock;
        private readonly ChaCha20EncryptionOptions _options;
        private readonly ChaCha20EncryptionProvider _provider;
        private readonly string _testDirectory;
        private readonly string _keyFile;

        public ChaCha20EncryptionProviderTests()
        {
            _loggerMock = new Mock<ILogger<ChaCha20EncryptionProvider>>();
            _testDirectory = Path.Combine(Path.GetTempPath(), $"chacha20_encryption_test_{Guid.NewGuid()}");
            Directory.CreateDirectory(_testDirectory);
            _keyFile = Path.Combine(_testDirectory, "chacha20_key.bin");
            _options = new ChaCha20EncryptionOptions
            {
                KeyFile = _keyFile
            };
            _provider = new ChaCha20EncryptionProvider(_loggerMock.Object, _options);
        }
        ...
        public async Task GetProviderInfo_ReturnsCorrectInfo()
        {
            ...
            // Assert
            Assert.Equal("ChaCha20", info.Name);
            Assert.Equal("ChaCha20-Poly1305", info.Algorithm);
            Assert.Equal(256, info.KeySizeBits);
            Assert.Equal(512, info.BlockSizeBits);
        ...
        public async Task InitializeAsync_WithProvidedKey_UsesProvidedKey()
        {
            ...
            var options = new ChaCha20EncryptionOptions
            {
                Key = key
            };
            var provider = new ChaCha20EncryptionProvider(_loggerMock.Object, options);
