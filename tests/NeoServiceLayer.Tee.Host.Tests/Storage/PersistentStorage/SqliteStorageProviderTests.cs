...
namespace NeoServiceLayer.Tee.Host.Tests.Storage.PersistentStorage
{
    ...
    {
        private readonly Mock<ILogger<SqliteStorageProvider>> _loggerMock;
        ...
        {
            _loggerMock = new Mock<ILogger<SqliteStorageProvider>>();
            _testDirectory = Path.Combine(Path.GetTempPath(), $"sqlite_storage_test_{Guid.NewGuid()}");
            Directory.CreateDirectory(_testDirectory);
            _databaseFile = Path.Combine(_testDirectory, "test.db");
            _options = new SqliteStorageOptions
            {
                DatabaseFile = _databaseFile
            };
            _provider = new SqliteStorageProvider(_loggerMock.Object, _options);
        }
