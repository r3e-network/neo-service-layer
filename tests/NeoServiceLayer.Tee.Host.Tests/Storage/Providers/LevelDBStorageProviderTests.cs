...
{
    public class LevelDBStorageProviderTests : IDisposable
    {
        private readonly Mock<ILogger<LevelDBStorageProvider>> _loggerMock;
        private readonly LevelDBStorageOptions _options;
        private readonly LevelDBStorageProvider _provider;
        private readonly string _testDirectory;

        public LevelDBStorageProviderTests()
        {
            _loggerMock = new Mock<ILogger<LevelDBStorageProvider>>();
            _testDirectory = Path.Combine(Path.GetTempPath(), $"leveldb_storage_test_{Guid.NewGuid()}");
            _options = new LevelDBStorageOptions
            {
                StoragePath = _testDirectory
            };
            _provider = new LevelDBStorageProvider(_loggerMock.Object, _options);
        }
