...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        // Add pattern recognition service
        services.AddScoped<IPatternRecognitionService, PatternRecognitionService>();

        // Add service discovery
        services.AddServiceDiscovery(_configuration, "pattern-recognition-service");

        // Add health checks
        services.AddHealthChecks()
            .AddCheck<ServiceHealthCheck>("pattern_recognition_health");
        ...
        {
            c.SwaggerDoc("v1", new() { Title = "Pattern Recognition Service API", Version = "v1" });
        });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILogger<Startup> logger)
    {
        if (env.IsDevelopment())
        {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Pattern Recognition Service API v1"));
        }
        ...
        {
            var patternService = scope.ServiceProvider.GetRequiredService<IPatternRecognitionService>();
            var initTask = patternService.InitializeAsync();
            initTask.Wait();
            ...
            {
                logger.LogError("Failed to initialize Pattern Recognition Service");
                throw new Exception("Service initialization failed");
            }
        }

        logger.LogInformation($"Pattern Recognition Service started on port {_configuration["Port"] ?? "8084"}");
    }
}
...
public class ServiceHealthCheck : Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck
{
    ...
    public ServiceHealthCheck(IPatternRecognitionService service)
    {
        _service = service;
    }
