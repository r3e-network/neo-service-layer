...
{
    /// <summary>
    /// A persistent storage provider that uses the file system for storage.
    /// </summary>
    public class FileStorageProvider : BasePersistentStorageProvider
    {
            ...
            : base(logger)
        {
            ...
            _options = options ?? new FileStorageOptions();
        ...
        public override async Task<bool> InitializeAsync()
        {
            ...
            try
            {
                _logger.LogInformation("Initializing file storage provider at {StoragePath}", _storagePath);
                ...
                _initialized = true;
                _logger.LogInformation("File storage provider initialized successfully");
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to initialize file storage provider");
                return false;
            }
        }
