...
{
    /// <summary>
    /// A persistent storage provider using RocksDB.
    /// </summary>
    public class RocksDBStorageProvider : BasePersistentStorageProvider
    {
        ...
        // Column family names
        private const string DataColumnFamily = "data";
        ...
        public RocksDBStorageProvider(ILogger<RocksDBStorageProvider> logger, RocksDBStorageOptions options = null)
            : base(logger)
        {
        ...
        {
            Logger.LogInformation("Initializing RocksDB storage provider with storage directory {StorageDirectory}", _options.StorageDirectory);
            ...
            try
            {
                ...
                try
                {
                    ...
                    columnFamilies.Add(DataColumnFamily, new ColumnFamilyOptions());
                    columnFamilies.Add(MetadataColumnFamily, new ColumnFamilyOptions());

                    // Open the database
                    _db = RocksDb.Open(options, _options.StorageDirectory, columnFamilies);

                    _initialized = true;
                    Logger.LogInformation("RocksDB storage provider initialized successfully");
                }
                finally
                {
                    Semaphore.Release();
            ...
            {
                Logger.LogError(ex, "Failed to initialize RocksDB storage provider");
                throw new StorageException("Failed to initialize RocksDB storage provider", ex);
            }
        }
        ...
        public override async Task<IReadOnlyList<string>> GetAllKeysAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Get all keys from the metadata column family
                    using (var iterator = _db.NewIterator(GetColumnFamily(MetadataColumnFamily)))
                    {
        ...
        public override async Task CompactAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _db.CompactRange(null, null, GetColumnFamily(DataColumnFamily));
                    _db.CompactRange(null, null, GetColumnFamily(MetadataColumnFamily));
        ...
        protected override Task WriteDataInternalAsync(string key, byte[] data)
        {
            ...
            _db.Put(GetDataKey(key), data, null, GetColumnFamily(DataColumnFamily));
        ...
        protected override Task<byte[]> ReadDataInternalAsync(string key)
        {
            ...
            byte[] data = _db.Get(GetDataKey(key), null, GetColumnFamily(DataColumnFamily));
        ...
        protected override Task DeleteDataInternalAsync(string key)
        {
            ...
            _db.Remove(GetDataKey(key), null, GetColumnFamily(DataColumnFamily));
            return Task.CompletedTask;
        }
        ...
        protected override Task<bool> ExistsInternalAsync(string key)
        {
            ...
            byte[] metadata = _db.Get(GetMetadataKey(key), null, GetColumnFamily(MetadataColumnFamily));
        ...
        protected override Task WriteMetadataInternalAsync(string key, StorageMetadata metadata)
        {
            ...
            _db.Put(GetMetadataKey(key), metadataBytes, null, GetColumnFamily(MetadataColumnFamily));
            return Task.CompletedTask;
        }
        ...
        protected override Task<StorageMetadata> ReadMetadataInternalAsync(string key)
        {
            ...
            byte[] metadataBytes = _db.Get(GetMetadataKey(key), null, GetColumnFamily(MetadataColumnFamily));
        ...
        protected override Task DeleteMetadataInternalAsync(string key)
        {
            ...
            _db.Remove(GetMetadataKey(key), null, GetColumnFamily(MetadataColumnFamily));
            return Task.CompletedTask;
        }
        ...
        {
            return _db.GetColumnFamily(name);
        }
