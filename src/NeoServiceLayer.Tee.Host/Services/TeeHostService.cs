...
namespace NeoServiceLayer.Tee.Host.Services
{
    ...
    public class TeeHostService : ITeeHostService, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the TeeHostService class with a custom enclave interface factory.
        /// </summary>
        /// <param name="configuration">The configuration.</param>
        /// <param name="logger">The logger.</param>
        /// <param name="enclaveFactory">The enclave interface factory to use.</param>
        public TeeHostService(IConfiguration configuration, ILogger<TeeHostService> logger, ITeeEnclaveFactory enclaveFactory)
        {
            ...
            // Create enclave interface using factory
            if (enclaveFactory != null)
            {
                var enclavePath = _configuration["Tee:Enclave:Path"] ?? "libenclave.so";
                _enclaveInterface = enclaveFactory.CreateEnclave(enclavePath, _isSimulationMode);
            }
            else
            {
                // Create enclave interface directly
                var loggerFactory = LoggerFactory.Create(builder => builder.AddConsole());

                // Use Occlum interface in all modes
                var enclaveLogger = loggerFactory.CreateLogger<OcclumInterface>();
                var enclavePath = _configuration["Tee:Enclave:Path"] ?? "libenclave.so";
                _enclaveInterface = new OcclumInterface(enclaveLogger, enclavePath);
            }
        ...
        public async Task<bool> InitializeTeeAsync()
        {
            try
            {
                ...
                // The enclave is already initialized in the constructor
                _isInitialized = true;

                // Get enclave measurements
                var mrEnclave = _enclaveInterface.GetMrEnclave();
                var mrSigner = _enclaveInterface.GetMrSigner();
        ...
        public async Task<TeeMessage> SendMessageAsync(TeeMessage message)
        {
            ...
            try
            {
                // Process the message in the enclave
                // Since we're using ITeeEnclaveInterface which doesn't have ProcessMessage,
                // we'll handle different message types directly
                TeeMessage response;
                ...
                switch (message.Type)
                {
                    case TeeMessageType.JavaScriptExecution:
                        ...
                        var jsResult = await _enclaveInterface.ExecuteJavaScriptAsync(
                    ...
                    case TeeMessageType.Attestation:
                        var attestationReport = _enclaveInterface.GetAttestationReport(new byte[0]);
                        var attestationData = new
                        {
                            Report = Convert.ToBase64String(attestationReport),
                            MrEnclave = Convert.ToBase64String(_enclaveInterface.GetMrEnclave()),
                            MrSigner = Convert.ToBase64String(_enclaveInterface.GetMrSigner())
                        };
