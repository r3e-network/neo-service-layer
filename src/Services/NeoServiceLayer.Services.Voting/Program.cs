...
namespace NeoServiceLayer.Services.Voting.Host
{
    /// <summary>
    /// Microservice host for VotingService
    /// </summary>
    public class VotingServiceHost : MicroserviceHost<VotingService>
    {
        public VotingServiceHost(string[] args) : base(args)
        {
        }
        ...
        protected override void ConfigureServiceSpecific(WebHostBuilderContext context, IServiceCollection services)
        {
            ...
            // services.Configure<VotingOptions>(configuration.GetSection("Voting"));

            // Add health checks
            services.AddHealthChecks()
                .AddCheck<VotingHealthCheck>("voting_service");
        }
        ...
        protected override void MapServiceEndpoints(Microsoft.AspNetCore.Routing.IEndpointRouteBuilder endpoints)
        {
            ...
            endpoints.MapGet("/api/voting/status", async context =>
            {
                var service = context.RequestServices.GetRequiredService<VotingService>();
                var status = new
                {
            ...
            endpoints.MapGet("/api/voting/statistics", async context =>
            {
                var service = context.RequestServices.GetRequiredService<VotingService>();
            ...
            endpoints.MapGet("/api/voting/metrics", async context =>
            {
...
# HELP voting_operations_total Total number of operations
# TYPE voting_operations_total counter
voting_operations_total{status=""success""} 0
voting_operations_total{status=""failed""} 0
";
        }
    }
    ...
    public class Program
    {
        public static async Task<int> Main(string[] args)
        {
            try
            {
                Console.WriteLine("Starting Voting Service...");

                var host = new VotingServiceHost(args);
                return await host.RunAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to start Voting Service: {ex.Message}");
                return 1;
            }
        }
    }
    ...
    public class VotingHealthCheck : Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck
    {
        private readonly VotingService _service;

        public VotingHealthCheck(VotingService service)
        {
            _service = service;
        }
