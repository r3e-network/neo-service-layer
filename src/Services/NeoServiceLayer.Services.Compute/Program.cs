...
{
    /// <summary>
    /// Microservice host for ComputeService
    /// </summary>
    public class ComputeServiceHost : MicroserviceHost<ComputeService>
    {
        public ComputeServiceHost(string[] args) : base(args)
        {
        }
        ...
        protected override void ConfigureServiceSpecific(WebHostBuilderContext context, IServiceCollection services)
        {
            ...
            services.AddHealthChecks()
                .AddCheck<ComputeHealthCheck>("compute_service");
        }
        ...
        protected override void MapServiceEndpoints(Microsoft.AspNetCore.Routing.IEndpointRouteBuilder endpoints)
        {
            ...
            endpoints.MapGet("/api/compute/status", async context =>
            {
                var service = context.RequestServices.GetRequiredService<ComputeService>();
                var status = new
                {
            ...
            endpoints.MapGet("/api/compute/statistics", async context =>
            {
                var service = context.RequestServices.GetRequiredService<ComputeService>();
            ...
            endpoints.MapGet("/api/compute/metrics", async context =>
            {
...
compute_operations_total{status=""failed""} 0
";
        }
    }
    ...
    public class Program
    {
        public static async Task<int> Main(string[] args)
        {
            try
            {
                Console.WriteLine("Starting Compute Service...");

                var host = new ComputeServiceHost(args);
                return await host.RunAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to start Compute Service: {ex.Message}");
                return 1;
            }
        }
    }
    ...
    public class ComputeHealthCheck : Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck
    {
        private readonly ComputeService _service;

        public ComputeHealthCheck(ComputeService service)
        {
            _service = service;
        }
