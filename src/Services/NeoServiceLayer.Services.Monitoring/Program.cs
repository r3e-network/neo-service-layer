...
namespace NeoServiceLayer.Services.Monitoring.Host
{
    /// <summary>
    /// Microservice host for MonitoringService
    /// </summary>
    public class MonitoringServiceHost : MicroserviceHost<MonitoringService>
    {
        public MonitoringServiceHost(string[] args) : base(args)
        {
        }
        ...
        protected override void ConfigureServiceSpecific(WebHostBuilderContext context, IServiceCollection services)
        {
            ...
            services.AddHealthChecks()
                .AddCheck<MonitoringHealthCheck>("monitoring_service");
        }
        ...
        protected override void MapServiceEndpoints(Microsoft.AspNetCore.Routing.IEndpointRouteBuilder endpoints)
        {
            ...
            endpoints.MapGet("/api/monitoring/status", async context =>
            {
                var service = context.RequestServices.GetRequiredService<MonitoringService>();
                var status = new
                {
            ...
            endpoints.MapGet("/api/monitoring/statistics", async context =>
            {
                var service = context.RequestServices.GetRequiredService<MonitoringService>();
            ...
            endpoints.MapGet("/api/monitoring/metrics", async context =>
            {
...
# HELP monitoring_operations_total Total number of operations
# TYPE monitoring_operations_total counter
monitoring_operations_total{status=""success""} 0
monitoring_operations_total{status=""failed""} 0
";
        }
    }
    ...
    public class Program
    {
        public static async Task<int> Main(string[] args)
        {
            try
            {
                ...
                var host = new MonitoringServiceHost(args);
                return await host.RunAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to start Monitoring Service: {ex.Message}");
                return 1;
            }
        }
    }
    ...
    public class MonitoringHealthCheck : Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck
    {
        ...
        public MonitoringHealthCheck(MonitoringService service)
        {
            _service = service;
        }
