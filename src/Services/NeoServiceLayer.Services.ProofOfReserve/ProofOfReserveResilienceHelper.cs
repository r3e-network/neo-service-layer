...
/// <summary>
/// Helper class providing retry and resilience patterns for the Proof of Reserve Service.
/// </summary>
public static class ProofOfReserveResilienceHelper
{
    /// <summary>
    ...
    public static async Task<T> ExecuteWithCircuitBreakerAsync<T>(
        Func<Task<T>> operation,
        ProofOfReserveCircuitBreaker circuitBreaker,
        ILogger logger,
        string operationName = "operation")
    {
    ...
    /// <summary>
    /// Executes blockchain operations with specialized retry logic.
    /// </summary>
    /// <typeparam name="T">The return type.</typeparam>
    /// <param name="operation">The blockchain operation to execute.</param>
    /// <param name="logger">The logger.</param>
    /// <param name="maxRetries">Maximum number of retries.</param>
    /// <param name="operationName">Name of the operation for logging.</param>
    /// <returns>The operation result.</returns>
    public static async Task<T> ExecuteBlockchainOperationAsync<T>(
        Func<Task<T>> operation,
        ILogger logger,
        int maxRetries = 5,
    ...
    /// <summary>
    /// Executes enclave operations with specialized retry logic.
    /// </summary>
    /// <typeparam name="T">The return type.</typeparam>
    /// <param name="operation">The enclave operation to execute.</param>
    /// <param name="logger">The logger.</param>
    /// <param name="maxRetries">Maximum number of retries.</param>
    /// <param name="operationName">Name of the operation for logging.</param>
    /// <returns>The operation result.</returns>
    public static async Task<T> ExecuteEnclaveOperationAsync<T>(
        Func<Task<T>> operation,
        ILogger logger,
        int maxRetries = 2,
        string operationName = "enclave operation")
    {
        return await ExecuteWithRetryAsync(
            operation,
            logger,
    ...
    /// <summary>
    /// Executes storage operations with specialized retry logic.
    /// </summary>
    /// <typeparam name="T">The return type.</typeparam>
    /// <param name="operation">The storage operation to execute.</param>
    /// <param name="logger">The logger.</param>
    /// <param name="maxRetries">Maximum number of retries.</param>
    /// <param name="operationName">Name of the operation for logging.</param>
    /// <returns>The operation result.</returns>
    public static async Task<T> ExecuteStorageOperationAsync<T>(
        Func<Task<T>> operation,
        ILogger logger,
        int maxRetries = 3,
    ...
    private static bool IsRetriableException(Exception exception)
    {
        return exception switch
        {
            ...
            InvalidOperationException ex when ex.Message.Contains("transient") => true,

            // Blockchain-related temporary failures
            InvalidOperationException ex when ex.Message.Contains("node") && ex.Message.Contains("unavailable") => true,
            InvalidOperationException ex when ex.Message.Contains("network") && ex.Message.Contains("error") => true,
            ...
            InvalidOperationException ex when ex.Message.Contains("attestation") && ex.Message.Contains("failed") => true,

            // Storage temporary failures
            InvalidOperationException ex when ex.Message.Contains("storage") && ex.Message.Contains("timeout") => true,
            InvalidOperationException ex when ex.Message.Contains("database") && ex.Message.Contains("busy") => true,

            // Proof of Reserve specific failures
    ...
    private static bool IsRetriableHttpStatusCode(HttpRequestException httpException)
    {
        ...
        return message.Contains("500") ||  // Internal Server Error
               ...
               message.Contains("522") ||  // Connection Timed Out
               message.Contains("524");    // A Timeout Occurred
    }
}

/// <summary>
/// Circuit breaker implementation for Proof of Reserve Service.
/// </summary>
public class ProofOfReserveCircuitBreaker
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ProofOfReserveCircuitBreaker"/> class.
    /// </summary>
    ...
    public ProofOfReserveCircuitBreaker(int failureThreshold = 5, TimeSpan? timeout = null)
    {
