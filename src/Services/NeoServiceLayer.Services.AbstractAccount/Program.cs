...
namespace NeoServiceLayer.Services.AbstractAccount.Host
{
    /// <summary>
    /// Microservice host for AbstractAccountService
    /// </summary>
    public class AbstractAccountServiceHost : MicroserviceHost<AbstractAccountService>
    {
        public AbstractAccountServiceHost(string[] args) : base(args)
        {
        }
        ...
        protected override void ConfigureServiceSpecific(WebHostBuilderContext context, IServiceCollection services)
        {
            ...
            // services.Configure<AbstractAccountOptions>(configuration.GetSection("AbstractAccount"));

            // Add health checks
            services.AddHealthChecks()
                .AddCheck<AbstractAccountHealthCheck>("abstract-account_service");
        }
        ...
        protected override void MapServiceEndpoints(Microsoft.AspNetCore.Routing.IEndpointRouteBuilder endpoints)
        {
            ...
            endpoints.MapGet("/api/abstract-account/status", async context =>
            {
                var service = context.RequestServices.GetRequiredService<AbstractAccountService>();
                var status = new
                {
            ...
            endpoints.MapGet("/api/abstract-account/statistics", async context =>
            {
                var service = context.RequestServices.GetRequiredService<AbstractAccountService>();
            ...
            endpoints.MapGet("/api/abstract-account/metrics", async context =>
            {
...
# HELP abstract-account_operations_total Total number of operations
# TYPE abstract-account_operations_total counter
abstract-account_operations_total{status=""success""} 0
abstract-account_operations_total{status=""failed""} 0
";
        }
    }
    ...
    public class Program
    {
        public static async Task<int> Main(string[] args)
        {
            try
            {
                Console.WriteLine("Starting AbstractAccount Service...");

                var host = new AbstractAccountServiceHost(args);
                return await host.RunAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to start AbstractAccount Service: {ex.Message}");
                return 1;
            }
        }
    }
    ...
    public class AbstractAccountHealthCheck : Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck
    {
        private readonly AbstractAccountService _service;

        public AbstractAccountHealthCheck(AbstractAccountService service)
        {
            _service = service;
        }
