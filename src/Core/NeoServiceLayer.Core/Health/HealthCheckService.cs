...
/// <summary>
/// Professional database health check for Neo Service Layer.
/// </summary>
public class NeoServiceDatabaseHealthCheck : IHealthCheck
{
    private readonly Func<Task<bool>> _checkDatabase;
    private readonly ILogger<NeoServiceDatabaseHealthCheck> _logger;

    public NeoServiceDatabaseHealthCheck(Func<Task<bool>> checkDatabase, ILogger<NeoServiceDatabaseHealthCheck> logger)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return isHealthy
                ? HealthCheckResult.Healthy("Neo Service Layer database connection is healthy")
                : HealthCheckResult.Unhealthy("Neo Service Layer database connection failed");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Neo Service Layer database health check failed");
            return HealthCheckResult.Unhealthy("Neo Service Layer database health check failed", ex);
        }
    }
}
